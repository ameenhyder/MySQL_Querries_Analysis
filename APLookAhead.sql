#Depending on MOBILE OR FIXED change the TABLE NAMES
Set @startDate = '2019-07-07';
Set @endDate = '2019-07-16';
#Set @skill = 18679;
Drop temporary table if exists tempdb.AP;
Create temporary table tempdb.AP
Select s.AgentID, s.BTN, s.V_CALLTIME, TIMESTAMPDIFF(Second, s.TalkStart,TalkEnd) AS HandleTime, s.RGUs, a.Percentile, a.IsCurrent
From `atentotfn_fixedsales.smetable` s
Left Join `atentotfn_fixedsales.agentperformance` a
On s.AgentID = a.AgentID
Where IsCurrent = 1 and V_CALLTIME >= @startDate and V_CALLTIME < @endDate and INSCOPE_FLAG = 1 and on_off = 0/* and skill = @skill*/;


Select @NoOfAgents := count(distinct AgentID)
From tempdb.AP;

Drop temporary table if exists tempdb.activeagents;
Create temporary table tempdb.activeagents
Select distinct AgentID, Percentile, IsCurrent
From tempdb.AP
Order By Percentile;

Set @num = 0;
Drop temporary table if exists tempdb.activeagents1;
Create temporary table tempdb.activeagents1
Select *, @num := @num + 1 AS Rank, @num/@NoOfAgents AS Repercentiled From tempdb.activeagents;

-- Select * from tempdb.activeagents1;

Drop temporary table if exists tempdb.AP2;
Create temporary table tempdb.AP2
Select s.AgentID, s.BTN, s.V_CALLTIME, TIMESTAMPDIFF(Second, s.TalkStart,TalkEnd) AS HandleTime, s.RGUs, s.skill, a.Percentile, a.Repercentiled
From `atentotfn_fixedsales.smetable` s
Left Join tempdb.activeagents1 a
On s.AgentID = a.AgentID
Where V_CALLTIME >= @startDate and V_CALLTIME < @endDate and IsCurrent = 1 and INSCOPE_FLAG = 1 and on_off = 0/*  and skill = @skill*/;
describe `atentotfn_fixedsales.smetable`;
Drop temporary table if exists tempdb.AP3;
Create temporary table tempdb.AP3
Select *,
	(CASE When Repercentiled between 0 and 0.2 then 1
		When Repercentiled between 0.2 and 0.4 then 2
        When Repercentiled between 0.4 and 0.6 then 3
        When Repercentiled between 0.6 and 0.8 then 4
        When Repercentiled between 0.8 and 1.0 then 5 END
    ) Buckets
From tempdb.AP2;

Select Buckets/*, Skill*/, (sum(RGUs)/count(RGUs)) AS AverageCR, count(distinct AgentID) AS NoOfAgents, count(BTN) AS NoOFCalls, (sum(HandleTime)/3600) AS `TotHandleTime(Hr)`
From tempdb.AP3
Group By Buckets;

########
describe `atentotfn_fixedsales.agentperformance`;
set sql_Safe_updates=0;
UPDATE `atentotfn_fixedsales.agentperformance`
SET Percentile = 0.5
Where AgentID in ('29888',
'29006',
'28530',
'27546',
'27380',
'26712',
'25651',
'25277',
'25061',
'24672',
'24409',
'24288',
'24191',
'24176',
'23658',
'23503',
'23452',
'23295',
'23177',
'22897',
'25346'
) and IsCurrent = 1
;
Select *
From `atentotfn_fixedsales.agentperformance`
Where AgentID in (29888,
29006,
28530,
27546,
27380,
26712,
25651,
25277,
25061,
24672,
24409,
24288,
24191,
24176,
23658,
23503,
23452,
23295,
23177,
22897,
25346
) and IsCurrent = 1
;

Select RGUs, Produto, Versao_Para, NUMERO_OS
From `atentotfn_fixedsales.smetable`;